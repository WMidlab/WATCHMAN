...
int main(){
	...
	xil_printf("\r\nDMA Test - with Interruption\r\n");
	xil_printf("Size UINTPR %d, Int %d, u8 %d\r\n",sizeof(UINTPTR),sizeof(int),sizeof(u8));
	...
	// --- Example ------------------------------------
	regbank[NBR_OF_PACKETS_REG] = PACKETS;
	regbank[CONTENT_PACKET_1] = 2000;
	
	
	xil_printf("Check RegBank\t%d\t%d\t%d\r\n",regbank[CONTROL_REG],regbank[NBR_OF_PACKETS_REG],regbank[CONTENT_PACKET_1]);
	
	// Old
	//PtrData = malloc(sizeof(int)*NBR_OF_PACKETS);
	if(PtrData == NULL){
	...
	for(int i=0; i < NBR_OF_PACKETS;i++){
		RxBufferPtr[i] = i;
	}*/
	/*
	for(int i=0; i < PACKETS;i++){
		...
		decToHexa(&PtrData[i]);
		xil_printf("\tData%d\t%d\r\n",i,PtrData[i]);
		//xil_printf("Addr: %d\tData%d\t%d\r\n",&RxBufferPtr[i],i,RxBufferPtr[i]);
	}
    */
	//xil_printf("Setup DMA Transfert %d\r\n",PACKETS*sizeof(int));	
	// Setup the DMA for Transfer
	Xil_DCacheFlushRange((UINTPTR)PtrData, PACKETS*sizeof(int));


//	Status = XAxiDma_SimpleTransfer(&AxiDma,(UINTPTR)RxBufferPtr, NBR_OF_PACKETS, XAXIDMA_DEVICE_TO_DMA);
	Status = XAxiDma_SimpleTransfer_Hej(&AxiDma,(UINTPTR)PtrData, (PACKETS*sizeof(int)));
//	Status = XAxiDma_SimpleTransfer_Hej(&AxiDma,(UINTPTR)RxBufferPtr, NBR_OF_PACKETS);
	...
	//xil_printf("Check RegBank\t%d\t%d\t%d\r\n",regbank[CONTROL_REG],regbank[NBR_OF_PACKETS_REG],regbank[CONTENT_PACKET_1]);
/*	xil_printf("Addr:\t");
		decToHexa(&regbank[CONTROL_REG]);
	xil_printf("\r\nAddr:\t");
		decToHexa(&regbank[NBR_OF_PACKETS_REG]);
	xil_printf("\r\nAddr:\t");
		decToHexa(&regbank[CONTENT_PACKET_1]);
	xil_printf("\r\n");	
*/		
	...
	/*xil_printf("Reg Length:\t");
	int n = XAxiDma_ReadReg(XPAR_AXI_DMA_0_BASEADDR+XAXIDMA_RX_OFFSET, XAXIDMA_BUFFLEN_OFFSET);
	decToHexa(n);
	xil_printf("\r\n");
	*/	
	//XAxiDma_S2MM_DMACR();
