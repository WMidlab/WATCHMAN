...
class Window(Frame):          
    ...
    def init_window(self):
        ...
        # Buttons
        self.btn_write_all = Button(self.master,text="Write all reg.", command=self.write_all_reg)
        self.btn_write_all.grid(column=4, row=0, rowspan=2, padx=5, sticky=W+E)
        self.btn_write_all.configure(state="disable")
        self.btn_read_all = Button(self.master,text="Read all reg.", command=self.read_all_reg)
        self.btn_read_all.grid(column=4, row=2, rowspan=2, padx=5, sticky=W+E)
        self.btn_ping = Button(self.master,text="Ping", command=self.send_ping)
        self.btn_ping.grid(column=4, row=4, rowspan=2, padx=5, sticky=W+E)
        self.btn_stream = Button(self.master,text="Start stream.", command=self.start_stream)
        self.btn_stream.grid(column=4, row=6, rowspan=2, padx=5, sticky=W+E)
        self.btn_stop = Button(self.master,text="Stop uC", command=self.stop_uC)
    ...
    def write_all_reg(self):
        self.write_txt("Command write_all_reg sent")
        cmd = 'wrall'
        for reg in self.regs:
            cmd = cmd + '/' + str(reg.get())
    ...
    def read_all_reg(self):
        self.write_txt("Command read_all_reg sent")
        #for reg in self.regs:
        #    print (reg.get())
        # do something with return value to recover reg's value
        # but also if timeout as occured
        self.send_command('rall')
    ...
    def start_stream(self):
        self.write_txt("Command start_stream sent")
        self.send_command('start_stream')

    def stop_uC(self):
        self.write_txt("Command stop_uC sent")
        self.send_command('stop_uC')
    ...
    def send_command(self, cmd):
        payload = 'head/' + cmd + '/end/'
        print(payload + str(random.randrange(0,1000)))
    ...
    def timer_int(self):
        while self.timer_flag:
            try:
                ...
                self.write_txt(data.decode('utf-8'))
            except socket.timeout:
                dummy=1 # dummy command just to use try
            time.sleep(0.5)
